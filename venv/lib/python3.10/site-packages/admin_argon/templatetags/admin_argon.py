...
from home.views import get_all_logged_in_users
...
def active_user():
    ...
    # user_count = User.objects.filter(id__in=uid_list).count()
    # user_queryset = User.objects.filter(id__in=uid_list)
    return User.objects.filter(id__in=uid_list).count(), User.objects.filter(id__in=uid_list) 
    # return user_count, user_queryset

@register.inclusion_tag('venv/lib/python3.10/site-packages/admin_argon/templates/admin/index.html')
